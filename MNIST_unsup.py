...
import numpy as np
import sys, os
from utils_u import Dataset, load_dataset
...
def train_network(net, trainloader, init_rate, step_size, gamma, total_epochs, weight_decay, testloader):
    ...
    optimizer = torch.optim.SGD(net.parameters(), lr=init_rate, weight_decay=weight_decay)
    ...
    for epoch in range(total_epochs):
        ...
        if np.mod(epoch,3) == 0:
            for i in range(4):
                id_ents[i].append(id_entropy(torch.squeeze(feats_all[i]).detach().cpu().numpy(),100,50))
...
def train_network_fin_id(net, trainloader, trainloader_small, init_rate, step_size, gamma, total_epochs, weight_decay, testloader):

    net = net
    net = net.cuda()
    net = net.train()
    # params = add_weight_decay(net, l2_normal,l2_special,name_special)
    optimizer = torch.optim.SGD(net.parameters(),momentum=0.9, lr=init_rate, weight_decay=weight_decay)
    ...
    for epoch in range(total_epochs):
        ...
        if epoch == total_epochs - 1:
            ...
            for i in range(1):
                id_ents.append(id_entropy(torch.squeeze(feats_all[i]).detach().cpu().numpy(),200,50))
...
def test_network(net, testloader, test_labels):
    ...
    for i in range(int(len(test_labels) / testloader.batch_size)):
        ...
        error = error + (torch.mean((outputs - images)**2)).detach().cpu().numpy()
        # _, predicted = torch.max(outputs, 1)
        # correct = correct + torch.sum(predicted == labels)
        torch.cuda.empty_cache()

    total_error = error/float(total)
    return total_error
    ...
    for idx in range(len(training_sizes)):
        ...
        for i in range(val_splits):
            ...
            if dataset_name == 'MNIST':
                ...
                testloader = torch.utils.data.DataLoader(Data_test, batch_size=10, shuffle=False, num_workers=0,
                                                         worker_init_fn=seed_worker)
            ...
            for j in range(len(Networks_to_train)):
                ...
                if no_reruns == 1:
                    ...
                    # noisy_probs = [0.2,0.5,0.8]
                    data_num = [500,1000,2000,2500,5000, 7500, 10000, 12500, 15000, 17500, 20000, 25000, 30000]
                    for temp in range(no_reruns):
                        # train_labels = listdict[i]['train_label']
                        # train_labels = noisy_labels(train_labels, noisy_probs[temp])
                        Data_train = Dataset(dataset_name, train_data[0: data_num[temp], :, :] , train_data[0: data_num[temp], :, :] , transform_train)
                        trainloader = torch.utils.data.DataLoader(Data_train, batch_size=batch_size, shuffle=False,
                                                                  num_workers=0, worker_init_fn=seed_worker)
                        # trainloader_small = torch.utils.data.DataLoader(Data_train, batch_size=20, shuffle=False,
                        #                                                 num_workers=0,
                        #                                                 worker_init_fn=seed_worker)
                        ...
                        for tempa in range(8):
                            id_ents, accuracy, accuracy2, net = train_network_fin_id(ConvAe(), trainloader, 0,
                                                                                     init_rate, step_size, gamma,
                                                                                     total_epochs, decay_normal,
                                                                                     testloader)
                            # print(id_ents)
                        ...
                        net = 0
                        # id_ents, accuracies, train_accuracies, net = train_network(Networks_to_train[0], trainloader, init_rate, step_size, gamma,
